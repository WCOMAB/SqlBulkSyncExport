...
Setup(
    static context =>
    {
        InstallTool("dotnet:https://api.nuget.org/v3/index.json?package=DPI&version=2026.8.13.465");
        InstallTool("dotnet:https://api.nuget.org/v3/index.json?package=GitVersion.Tool&version=6.8.2");
        ...
        context.Information(
            "Building version {0} (Branch: {1}, IsMain: {2})",
            version,
            branchName,
            isMainBranch);
        ...
        var projectPath = projectRoot.CombineWithFilePath("SqlBulkSyncExport/SqlBulkSyncExport.csproj");
        ...
        return new BuildData(
            ...
            new DotNetMSBuildSettings()
                ...
                .WithProperty("PackageTags", "tool;sql;change-tracking;csv;export")
                .WithProperty(
                    "PackageDescription",
                    "SqlBulkSyncExport .NET Tool – Exports SQL Server change-tracking (and optional full-sync) deltas to CSV files in a target folder.")
                .WithProperty("RepositoryUrl", "https://github.com/WCOMAB/SqlBulkSyncExport.git")
                .WithProperty(
                    "ContinuousIntegrationBuild",
                    GitHubActions.IsRunningOnGitHubActions ? "true" : "false")
 ...
 *****************************/
Task("Clean")
    .Does<BuildData>(static (context, data) => context.CleanDirectories(data.DirectoryPathsToClean))
.Then("Restore")
    .Does<BuildData>(
...
.Then("Test")
    .Does<BuildData>(
        static (context, data) => context.DotNetTest(
            data.ProjectRoot.CombineWithFilePath("SqlBulkSyncExport.slnx").FullPath,
...
.Then("Push-GitHub-Packages")
    .WithCriteria<BuildData>((_, data) => data.ShouldPushGitHubPackages())
...
.Then("Push-NuGet-Packages")
    .WithCriteria<BuildData>((_, data) => data.ShouldPushNuGetPackages())
    .DoesForEach<BuildData, FilePath>(
        static (data, context) => context.GetFiles(data.NuGetOutputPath.FullPath + "/*.nupkg"),
        static (data, item, context) => context.DotNetNuGetPush(
            item.FullPath,
